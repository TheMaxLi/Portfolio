<script lang="ts">
	import { Spring } from 'svelte/motion';
	import { fade } from 'svelte/transition';
	import { hideTooltip, tooltipState } from '$lib/utils/tooltip.svelte';
	import { page } from '$app/state';

	let tooltipPosition = $state({
		x: 0,
		y: 0
	});

	let coords = new Spring(
		{ x: 0, y: 0 },
		{
			stiffness: 0.1,
			damping: 0.25
		}
	);

	let currentPath = $state('');
	$effect(() => {
		if (page && page.url.pathname !== currentPath) {
			currentPath = page.url.pathname;
			hideTooltip();
		}
	});

	function handleMouseMove(event: MouseEvent) {
		if (tooltipState.isVisible) {
			coords.target = { x: event.clientX, y: event.clientY };
		}
	}

	$effect(() => {
		coords;
		tooltipPosition = {
			x: coords.current.x + 15,
			y: coords.current.y + 15
		};
	});

	$effect(() => {
		window.addEventListener('mousemove', handleMouseMove);

		return () => {
			window.removeEventListener('mousemove', handleMouseMove);
		};
	});
</script>

{#if tooltipState.isVisible}
	<div
		class="pointer-events-none fixed z-[9999] max-w-[550px] border bg-black px-[10px] py-[6px] text-sm text-[#CFD1CF] shadow [text-shadow:_0px_0px_10px_rgb(255_255_255_/_1.00)] max-lg:hidden"
		style="left: {tooltipPosition.x}px; top: {tooltipPosition.y}px;"
		transition:fade={{ duration: 150 }}
	>
		{tooltipState.text}
	</div>
{/if}
